			...
			<div class="review" v-if="comment.comment_id">
				<div class="review_info">
					<div class="review_portrait">
						<img :src="comment.img_head || '../../static/images/moren@3x.png'">
			...
			created() {
				...
				this.$api.goodsDetail({
					...
					if(res.ret == 1) {
						...
						this.getCartData()
						this.lineLink = 'http://mobile.jxdyf.cn/' + window.location.hash
			...
			methods: {
				...
				goodsOperate(status) {
					...
					if(!token.getAccessToken()) {
						this.$storage.set('history_url', this.$route.fullPath)
						this.$router.replace('/appLogin')
						return
					}
				...
				operate() {
					...
					} else if(this.opereteStatus === 'pay') {

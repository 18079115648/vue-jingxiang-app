<template>
    <div class="app">
    	...
    	<div class="pay-content">
    		...
    		<div class="pay-btn" v-if="type_id == 901 && !isWx" @click.stop="payApp">微信支付</div>
...
//          })
		...
		onFileChange(e) {
            ...
            axios.post('/index/order/img_user', param, {
            	headers:{
		...
		changeAvatar() {
			...
			let data = {
				'TOKEN': this.$token.getAccessToken(),
				'type': 2,
				'url': 'http://' + window.location.host + '/index/order/img_user'
			}
			Indicator.open()
			this.$bridge.choosePhoto(data).then(res => {
				Indicator.close()
					...
					this.$api.appPay({
						...
						if(msg.ret == 1) {
							this.$bridge.wechatPay({
								...
								this.disabledBtn = false
								if(data.ret == 1) {
									self.$router.replace('/orderDetail/' + res.order_id)
								}else {
									self.$router.go(-1)
								}
